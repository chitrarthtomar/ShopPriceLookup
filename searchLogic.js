...
const fetchItems = async () => {
    products = await fetch('http://127.0.0.1:8887/products.json').then(
		res => res.json()
	);
}
...
const showItems = async () => {
	...
	products.filter(
		product => product.name.toLowerCase().includes(search_term.toLowerCase())
	).forEach(product => {
...
document.getElementById("save-details-button").addEventListener("click", (e) => {
